...
from models.actor_sex import ActorSex


class ActorSexController(BaseController):
    """"""
    ...
    def get(self, *args, **kwargs):
        ...
        try:
            ...
            if len(id_list) == 1 and id_list[0].lower() == 'all':
                res = self.query(ActorSex).all()
            else:
                res = self.query(ActorSex).filter(ActorSex.id.in_(id_list)).all()
            self.do_write([self.pack_sex_info(sex) for sex in res])
        except Exception, e:
    ...
    def pack_sex_info(self, sex):
        """
        在这里把所有需要返回的字段写入一个dict
        """
        print sex
        return dict(id=sex.id, name=sex.name, detail=sex.detail)
